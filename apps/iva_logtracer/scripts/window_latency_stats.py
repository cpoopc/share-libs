...
CP_TOOLS_ROOT = Path(__file__).resolve().parents[3]
OUTPUT_SESSION_ROOT = APP_ROOT / "output" / "iva_session"
DIAGNOSTIC_REPORT_PATH = (
    CP_TOOLS_ROOT / "agents" / "skills" / "iva-logtracer" / "scripts" / "diagnostic_report.py"
)
...
def _load_diagnostic_report_module() -> Any:
    spec = importlib.util.spec_from_file_location("diagnostic_report", DIAGNOSTIC_REPORT_PATH)
    module = importlib.util.module_from_spec(spec)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load diagnostic report module from {DIAGNOSTIC_REPORT_PATH}")
...
def _load_session_diagnostics(
    *,
    ...
    diagnostic_report = _load_diagnostic_report_module()
