...
```bash
# 追踪 session（自动保存到 output/iva_session/）
...
```bash
# 分析 session 目录
iva-logtracer turn ./output/iva_session/s-xxx-yyy

# Markdown 格式输出
iva-logtracer turn ./output/iva_session/s-xxx-yyy --format markdown

# 导出报告
iva-logtracer turn ./output/iva_session/s-xxx-yyy -o turn_report.json
```
