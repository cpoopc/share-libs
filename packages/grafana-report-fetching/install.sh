...
SKILL_NAME="grafana-report-fetching"
REMOTE_PACKAGE_SOURCE="git+ssh://git@github.com/cpoopc/share-libs.git#subdirectory=packages/grafana-report-fetching"
...
usage() {
    cat <<'EOF'
Install grafana-report-fetching CLI and the optional Codex-compatible skill.
...
Options:
  ...
  --skip-init     Skip `grafana-report-fetching init`
...
if [ "$RUN_INIT" -eq 1 ]; then
    ...
    run_cmd uv tool run --from "$PACKAGE_SOURCE" grafana-report-fetching init
fi
...
log "  2. Run: grafana-report-fetching doctor --profile iva --real"
log "  3. Run: grafana-report-fetching resolve-profile iva-prod"
