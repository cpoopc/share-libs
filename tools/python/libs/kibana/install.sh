...
SKILL_NAME="kibana"
REMOTE_PACKAGE_SOURCE="git+ssh://git@github.com/cpoopc/share-libs.git#subdirectory=tools/python/libs/kibana"
...
usage() {
    cat <<'EOF'
Install kibana-query CLI and the optional Codex-compatible skill.
...
Options:
  ...
  --skip-init     Skip `kibana-query init`
...
if [ "$RUN_INIT" -eq 1 ]; then
    ...
    run_cmd uv tool run --from "$PACKAGE_SOURCE" kibana-query init
    ensure_env_variants
fi
...
log "  2. Run: kibana-query doctor"
log "  3. Run: kibana-query search 'level:ERROR' --env production --last 1h"
